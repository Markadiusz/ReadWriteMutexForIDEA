...
internal abstract class ConcurrentLinkedListNode<N : ConcurrentLinkedListNode<N>>(prev: N?) {
     ...
     */
    fun remove() {
        @Suppress("INVISIBLE_REFERENCE", "INVISIBLE_MEMBER") assert { isRemoved || isTail } // The node should be logically removed at first.
    ...
    private val aliveSegmentRight: N get() {
        @Suppress("INVISIBLE_REFERENCE", "INVISIBLE_MEMBER") assert { !isTail } // Should not be invoked on the tail node
        var cur = next!!
        while (cur.isRemoved)
...
internal abstract class Segment<S : Segment<S>>(val id: Long, prev: S?, pointers: Int) :
    ...
    @Suppress("INVISIBLE_REFERENCE", "INVISIBLE_MEMBER") NotCompleted
{
    /**
...
private val CLOSED = @Suppress("INVISIBLE_REFERENCE", "INVISIBLE_MEMBER") Symbol("CLOSED")